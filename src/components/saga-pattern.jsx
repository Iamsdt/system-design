...
export default function SagaPattern() {
  ...
  const patterns = {
    choreography: {
      ...
      description:
        "Each service listens to events and knows what to do next. Decentralized coordination.",
      architecture: `
┌──────────────────────────────────────────────────┐
...
// No central coordinator; flow is implicit`,
    ...
    orchestration: {
      ...
      description:
        "Central coordinator tells each service what to do. Centralized control flow.",
      architecture: `
┌──────────────────────────────────────────────────┐
...
async function runSaga(order) {
  ...
  const simulationSteps = {
    choreography: [
      ...
      },
      {
        step: 4,
        service: "Payment",
        action: "Refund Payment (Compensation)",
        status: "success",
      ...
      },
      {
        step: 5,
        service: "Order",
        action: "Cancel Order (Compensation)",
        status: "success",
  ...
  return (
    <div className="space-y-8">
      <div>
        <h3 className="text-3xl font-bold text-slate-900 mb-3">Saga Pattern</h3>
        <p className="text-lg text-slate-600 mb-6">
          Managing distributed transactions across microservices. When ACID
          isn't possible, use sagas for eventual consistency.
        </p>
      </div>
        ...
        <div className="flex items-start gap-4 mb-6">
          ...
          <div>
            ...
            <p className="text-slate-700 mb-4">
              In microservices, you can't use database transactions across
              services. Consider an e-commerce order:
            </p>
            <div className="bg-white rounded-lg p-4 border border-red-200 mb-4">
              <div className="space-y-2 text-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="font-bold">Order Service:</span> Create
                    order record
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="font-bold">Payment Service:</span> Charge
                    customer's card
                  </div>
                </div>
                <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="font-bold">Shipping Service:</span> Arrange
                    delivery
                  </div>
                </div>
              </div>
            ...
            <p className="text-slate-700">
              ...
              You've already charged the customer! You need to{" "}
              <span className="font-bold">compensate</span> (refund payment,
              cancel order). This is where Sagas help.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {Object.entries(patterns).map(([key, pattern]) => (
          ...
          >
            <div
              className={`mb-3 ${selectedPattern === key ? "text-blue-600" : "text-slate-600"}`}
            >
              {pattern.icon}
            </div>
        ...
        <div className="bg-white rounded-xl p-6 border border-purple-200">
          <div className="space-y-3">
            {steps.map((step, index) => {
              const isActive =
                index === currentStep && simulationState === "running"
              const isComplete =
                index < currentStep || simulationState === "completed"
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    {isComplete && step.status === "compensating" && (
        ...
        {simulationState === "completed" && (
          <div className="mt-4 bg-red-100 border-2 border-red-300 rounded-lg p-4">
            ...
            <div className="text-sm text-red-700 mt-2">
              The saga detected inventory failure and automatically triggered
              compensation steps to undo all previous operations. This ensures
        ...
        <div className="space-y-4">
          <div className="border-l-4 border-blue-500 pl-4">
            ...
            <p className="text-sm text-slate-600 mb-2">
              Mark resources as "pending" during saga, release or finalize on
...
item.status = 'AVAILABLE'`}
          ...
          <div className="border-l-4 border-green-500 pl-4">
            ...
            <p className="text-sm text-slate-600 mb-2">
              Use operations that can be applied in any order (add/subtract vs
              set)
            </p>
            <pre className="text-xs bg-slate-900 text-slate-100 p-3 rounded">
              {`// ✓ Good: Add/Subtract
...
balance = 1000  // Can't reverse`}
          ...
          <div className="border-l-4 border-purple-500 pl-4">
            ...
            <p className="text-sm text-slate-600 mb-2">
              Identify the point of no return where compensation is no longer
              possible
            </p>
            <pre className="text-xs bg-slate-900 text-slate-100 p-3 rounded">
              {`// Reversible steps
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-sm text-slate-700">
              <span className="font-semibold">A:</span> "Neither is universally
              better. Orchestration is easier to understand and debug, good for
              complex workflows with strict ordering (e.g., loan approval).
              Choreography scales better and gives services more autonomy, good
              for event-driven systems (e.g., social media notifications). I'd
              start with orchestration for simplicity, switch to choreography
              when services need independence. Netflix uses choreography, Uber
              uses orchestration."
            </p>
          </div>
          <div>
            ...
            <p className="text-sm text-slate-700">
              <span className="font-semibold">A:</span> "Store saga state
              durably (database or event log). If orchestrator crashes, it can
              resume from checkpoint. Ensure compensation operations are
              idempotent—they might run multiple times. Use timeouts to detect
              stuck sagas (e.g., if payment takes &gt;30s, trigger
              compensation). Monitor saga completion rates and have alerts for
              long-running or failed sagas."
            </p>
          </div>
          <div>
            <p className="font-semibold text-slate-900 mb-2">
            ...
            <p className="text-sm text-slate-700">
              <span className="font-semibold">A:</span> "This is the hardest
              case. Retry with exponential backoff. If it keeps failing, mark
              saga as 'needs manual intervention' and alert on-call. Some
              compensations might not be possible (e.g., email already sent)—in
              that case, send a correction (refund email, apology). Design
              compensations to be simple operations that rarely fail (e.g.,
              database updates, not external API calls)."
            </p>
          </div>
        </div>
      </div>
